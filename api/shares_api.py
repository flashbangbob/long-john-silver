...
def get_all_shares_for_corp_id(id):
    ...
    with con:
        cur = con.cursor()
        cur.execute("SELECT corporation_id, count(*) as quantity, avg(purchase_price) as purchase_price, c.share_price, avg(purchase_price) - c.share_price as net_gain  FROM shares s
